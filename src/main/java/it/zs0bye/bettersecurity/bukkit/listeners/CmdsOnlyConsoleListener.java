...
public class CmdsOnlyConsoleListener implements Listener {
    ...
    public CmdsOnlyConsoleListener(final BetterSecurityBukkit plugin) {
        ...
        if(!Command.COMMANDS_ONLY_CONSOLE_ENABLED.getBoolean()) return;
        this.plugin.getServer().getPluginManager().registerEvent(
                PlayerCommandPreprocessEvent.class,
                this,
                EventPriority.valueOf(Command.COMMANDS_ONLY_CONSOLE_PRIORITY.getString().toUpperCase()),
    ...
    public void onCommandPreprocess(final PlayerCommandPreprocessEvent event) {
        ...
        if(!Command.COMMANDS_ONLY_CONSOLE.getStringList().contains(command)) return;

        this.placeholders.put("%player%", player.getName());
        this.placeholders.put("%command%", "/" + command);

        SendExecutors.send(this.plugin, Command.COMMANDS_ONLY_CONSOLE_EXECUTORS.getStringList(), player, this.placeholders);
        event.setCancelled(true);

        if(!Command.COMMANDS_ONLY_CONSOLE_WARNING.getBoolean()) return;
