...
public class BlocksCmdsListener implements Listener {
    ...
    public BlocksCmdsListener(final BetterSecurityBukkit plugin) {
        ...
        if(!Command.BLOCKS_COMMANDS_ENABLED.getBoolean()) return;
        this.plugin.getServer().getPluginManager().registerEvent(
                PlayerCommandPreprocessEvent.class,
                this,
                EventPriority.valueOf(Command.BLOCKS_COMMANDS_PRIORITY.getString().toUpperCase()),
    ...
    public void onCommandPreprocess(final PlayerCommandPreprocessEvent event) {
        ...
        SendExecutors.send(this.plugin, Command.BLOCKS_COMMANDS_EXECUTORS.getStringList(), player, this.placeholders);
        event.setCancelled(true);

        if(!Command.BLOCKS_COMMANDS_WARNING.getBoolean()) return;
        new Warnings(this.plugin, player, TypeWarning.COMMANDS, command);
    }

    private boolean canBlock(final String command) {
        final String method = Command.BLOCKS_COMMANDS_METHOD.getString();
