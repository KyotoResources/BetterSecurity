...
public class MainCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(@NotNull CommandSender sender, Command cmd, @NotNull String commandLabel, String[] args) {
        ...
        final BetterUser user = new BukkitUser(sender);
        if(!cmd.getName().equalsIgnoreCase(this.command)) return true;

        if(!sender.hasPermission(this.command + ".command")) {
            new AboutSubCMD(this.command, user, this.plugin);
            return true;
        }

        if(args.length == 0) {
            new HelpSubCMD(this.command, user, commandLabel, this.plugin);
            return true;
        }

        if(args.length == 1) {
            new ReloadSubCMD(this.command, args, user, this.plugin);
            new AboutSubCMD(this.command, args, user, this.plugin);
        ...
        if(args.length == 2) {
            new HelpSubCMD(this.command, args, user, commandLabel, this.plugin);
    ...
    public List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command cmd, @NotNull String commandLabel, @NotNull String[] args) {
        ...
        if(args.length == 2) {
            new HelpSubCMD(this.command, commands, args, sender);
