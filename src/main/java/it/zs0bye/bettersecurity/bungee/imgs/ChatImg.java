package it.zs0bye.bettersecurity.bungee.imgs;

import it.zs0bye.bettersecurity.bungee.imgs.enums.ImageColor;
...
public class ChatImg {
    ...
    public ChatImg(final BufferedImage image, final int height, final char imgChar) {
        ImageColor[][] chatColors = this.toChatColorArray(image, height);
    ...
    private ImageColor[][] toChatColorArray(BufferedImage image, int height) {
        double ratio = (double) image.getHeight() / image.getWidth();
        BufferedImage resized = resizeImage(image, (int) (height / ratio), height);

        ImageColor[][] chatImg = new ImageColor[resized.getWidth()][resized.getHeight()];
        for (int x = 0; x < resized.getWidth(); x++) {
            for (int y = 0; y < resized.getHeight(); y++) {
                int rgb = resized.getRGB(x, y);
                ImageColor closest = getClosestChatColor(new Color(rgb, true));
                chatImg[x][y] = closest;
            }
        }
        return chatImg;
    ...
    private String[] toImgMessage(ImageColor[][] colors, char imgchar) {
        String[] lines = new String[colors[0].length];
        for (int y = 0; y < colors[0].length; y++) {
            final StringBuilder line = new StringBuilder();
            for (ImageColor[] chatColors : colors) {
                final ImageColor color = chatColors[y];
                line.append((color != null) ? chatColors[y].toString() + imgchar : TRANSPARENT_CHAR);
            }
            lines[y] = line.toString() + ImageColor.RESET;
        }
        return lines;
    }
    ...
    private ImageColor getClosestChatColor(Color color) {
    ...
    public void send(final CommandSender sender) {
        ...
        for (final String line : lines) StringUtils.send(sender, "  " + StringUtils.colorize(line));
        sender.sendMessage(new TextComponent(""));
    }
}
